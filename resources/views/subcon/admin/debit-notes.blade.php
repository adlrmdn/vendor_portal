...
<form method="GET" action="{{ route('subcon.admin.debit-notes') }}" class="card border-0 shadow-sm mb-3">
    <div class="card-body">
        <div class="row g-2 align-items-end">
            ...
            <div class="col-sm-3 d-flex gap-2">
                ...
                @if($hasFilters)
                    <a href="{{ route('subcon.admin.debit-notes') }}" class="btn btn-sm btn-outline-secondary" title="Clear filters"><i class="fas fa-times"></i></a>
                @endif
            </div>
        </div>
    </div>
            ...
            <tbody>
                @forelse ($rows as $row)
                    <tr>
                        ...
                        <td>
                            @if ($row->has_report)
                                <a href="{{ route('subcon.admin.debit-notes.report', $row->id) }}"
                ...
                @empty
                    <tr>
                        <td colspan="9" class="text-center text-muted py-5">No debit notes match{{ $hasFilters ? ' these filters' : '' }}.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
