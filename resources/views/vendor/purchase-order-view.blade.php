...
@section('content')
    ...
    <div class="container-fluid">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <a href="{{ route('vendor.purchase-orders') }}" class="btn btn-sm btn-outline-secondary mb-2">
                            ...
                            <div class="d-flex justify-content-between align-items-center mb-1">
                                ...
                                <div>
                                    @if($item->status == 'pending')
                                        <a href="{{ route('vendor.item.process', $item->id) }}"
                                    ...
                                    @elseif($item->status == 'processing')
                                        <div class="btn-group">
                                            ...
                                            <form action="{{ route('vendor.item.mark-processed', $item->id) }}" method="POST"
                                                class="d-inline">
                                                @csrf
                                    ...
                                    @elseif($item->status == 'completed')
                                        <div class="btn-group">
                                            ...
                                            <button type="button" class="btn btn-sm btn-warning" style="min-width: 100px;"
                                                ...
                                                data-action="{{ route('vendor.item.revert-processing', $item->id) }}">
        ...
        <div class="modal fade" id="generateSlipModal" tabindex="-1">
            <div class="modal-dialog">
                <div class="modal-content">
                    <form action="{{ route('vendor.generate-packing-slip', $purchaseOrder->id) }}" method="POST"
                        target="_blank">
                        @csrf
